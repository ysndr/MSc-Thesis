...
Since the IntelliJ platform is based on the JVM, its plugin system requires the use of JVM languages[@cebronCustomLanguageSupport], making it hard to reuse the code of e.g. a reference compiler or interpreter.
...
Compiler frontends for different languages -- e.g. Clang[@lattnerLLVMClangNext2008], Rustc[@matsakisRustLanguage2014], NVCC[@nvidiaCUDACompilerDriver2022],... -- compile input languages into LLVM IR, a low level language with additional capabilities to provide optimization hints but independent of a particular architecture.
...
1. Three complete implementations that tightly integrate with the implementation level tooling of the respective language:
   *rust-analyzer[@therustprogramminglanguageBringingGreatIDE], *ocaml-lsp*/*merlin[@OCamlLSP2022,@bourMerlinLanguageServer2018a] and the *Haskell Language Server[@mitchellBuildingIntegratedDevelopment2020]
...
4. Two projects that facilitate the LSP as an interface to an existing tool via HTTP or command line.
   *CPAchecker[@beyerCPAcheckerToolConfigurable2009,@leimeisterLanguageServerIDE2020] and *CodeCompass[@porkolabCodecompassOpenSoftware2018]
...
An instance of this type is the rnix-lsp[@Rnixlsp2022] language server for the Nix[@dolstraNixSafePolicyFree2004] programming language.
...
While DSL creation workbenches like Xtext[@bettiniImplementingDomainspecificLanguages2016], Spoofax[@katsSpoofaxLanguageWorkbench2010] or MPS[@pechJetBrainsMPSWhy2021] allow for the implementation and provision of Eclipse or IntelliJ based DSLs, tooling for these languages is usually tied to the underlying platform.
...
It is used in products like VSCode, Theia[@vogeleclipse] and other web accessible code editors.
...
Nowadays, code hosting platforms are an integral part of the developer tool set (GitHub, Sourcegraph, GitLab, Sourceforge, etc.). 
